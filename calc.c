		...
		{	
			...
			{	//printf("berechnet wird score für: %s\n",treeN[treePosN[parent_p->c[sib_u1]]].o);
				...
				for (int k=0;k<=kMax;k++)
				{
					...
					float sum_j=0.;	//lt. recursion: summe über alle j´s
					float prod_sib_w=1.; //lt. recursion: hält prod über s
					float sum_i=0.;
					...
					for (int i=0; i<=kMax; i++)
					{
						...
						{	
							...
							{	//printf("geschwister von %s die beachtet werden: %s\n",treeN[treePosN[parent_p->c[sib_u1]]].o,treeN[treePosN[parent_p->c[sib_u]]].o);		//passt
								...
								//printf("* sum_j %f = prod_sib_w %f\n\n",sum_j, prod_sib_w);	
								sum_j=0.;
							}
								
						}
						
						sum_i=sum_i+Zaussen[i][parent_zeile]*exp(( (-1/bet)*abs(i-k)))*prod_sib_w;
										//~ printf("\n\nZaussen[%d][parent_zeile] %f  *e^( -%d)  %f",i, Zaussen[i][parent_zeile], abs(i-k), Zaussen[i][parent_zeile]*exp((bet*abs(i-k)))); 
										//~ printf("* prod_sib_w %f =  sum_i %f \n\n",prod_sib_w,i,sum_i);
						//printf prod_sib_w,i,sum_i);
						prod_sib_w = 1.;
					...
					//~ {				
						//~ sum_Z_aussen= sum_Z_aussen + sum_i[l];
						//~ //printf("sum_Z_aussen: %f \n",sum_Z_aussen);
					//~ }
						Zaussen[k][treePosN[parent_p->c[sib_u1]]] = sum_i*Z[k][treePosN[parent_p->c[sib_u1]]];
						sum_i=0;
			    ...
			    {
					if (treeN[treePosN[parent_p->c[sib_skalieren]]].c[0]==-1)
			    	{continue;}
			    	...
			    	{
			    		tempsum=tempsum + Zaussen[pk][treePosN[parent_p->c[sib_skalieren]]];	
...
void gl_printTreePSN(int kMax, int lca, struct gl_arguments ga) {
  ...
  {		if (ga.collectfile == NULL)
	...
	fprintf(T, "%%!PS-Adobe-2.1 EPSF-2.0\n");			
	fprintf(T, "%%"); fprintf(T, "%%Title: Efficient Prediction of Paralog Evolution - %s\n",ga.psfile);
	fprintf(T, "%%"); fprintf(T, "%%Creator: ePoPE 2.0 -- by Gabor Balogh, Jana Hertel\n");
	fprintf(T, "%%"); fprintf(T, "%%DocumentFonts: Helvetica\n");
	fprintf(T, "%%"); fprintf(T, "%%BoundingBox: 0 0 595 842\n");
	fprintf(T, "%%"); fprintf(T, "%%EndComments\n\n");
